...
export class StripeService {
  ...
  constructor(readonly configService: ConfigService){
    this.stripe = new Stripe(configService.get<string>('STRIPE_SECRET_KEY'), { apiVersion: '2020-08-27'});
  }

  async createPaymentIntent(orderId: string, totalAmount: number): Promise<Stripe.PaymentIntent>{
    try { 
      ...
      Logger.error('[stripeService] Error creating a payment intent', util.inspect(error));
